...
def delete_old_chats(user_email, limit=10):
    """Keep only the latest `limit` chats per user"""
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()

    # Get latest N chat_ids
    c.execute('''
        SELECT DISTINCT chat_id FROM chat_history
        WHERE user_email = ?
        ORDER BY timestamp DESC
        LIMIT ?
    ''', (user_email, limit))
    latest_chat_ids = [row[0] for row in c.fetchall()]

    # Delete all other chats
    if latest_chat_ids:
        placeholders = ','.join(['?'] * len(latest_chat_ids))
        query = f'''
            DELETE FROM chat_history
            WHERE user_email = ?
            AND chat_id NOT IN ({placeholders})
        '''
        c.execute(query, (user_email, *latest_chat_ids))
        conn.commit()
